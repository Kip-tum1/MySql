...
FROM subscriptions;
-- 2 What is the monthly distribution of trial the plan start_date values for our dataset - use the start of the month as the group by value.
SELECT plan_id, MONTH(start_date) AS month,
     COUNT(MONTH(start_date)) AS COUNT 
     FROM subscriptions 
     GROUP BY MONTH(start_date), plan_id
     HAVING plan_id = 0;
-- 3. What plan start_date do values occur after the year 2020 for our dataset? Show the breakdown by count of events for each plan_name.
SELECT plans.plan_id, plans.plan_name, year(start_date),
COUNT( year(start_date)) AS count_year
FROM subscriptions
JOIN plans ON plans.plan_id = subscriptions.plan_id
WHERE year(start_date) > 2020
GROUP BY plans.plan_id, plans.plan_name, year(start_date);
...
FROM subscriptions where plan_id = 4 and customer_id IN (SELECT customer_id FROM subscriptions WHERE plan_id = 0);
...
SELECT COUNT(DISTINCT subscriptions.customer_id) AS num_customers
FROM subscriptions INNER JOIN plans 
ON subscriptions.plan_id = plans.plan_id
WHERE plans.plan_name = "pro annual"  AND YEAR(subscriptions.start_date) = 2020;
...
SELECT plan_id, COUNT(DISTINCT customer_id) AS downgraded_from_annual
FROM subscriptions
WHERE plan_id =1 AND customer_id IN (SELECT DISTINCT customer_id FROM subscriptions WHERE plan_id = 2 AND YEAR(start_date) = 2020)
